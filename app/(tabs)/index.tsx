...
export default function OnThisDay() {
  ...
  const loadVault = async () => {
    ...
    for (const dateKey of dateKeySet) {
      ...
      if (!context && photos.length === 0 && !presentEntry) continue;
      ...
      days.push({
        dateKey, displayDate, context, presentEntry, photos,
  ...
  const setAsCover = async (memory: Memory) => {
    ...
    setMemoryList(prev => {
      const updated = prev.map(m => ({
        ...m,
        isCover: m.year === memory.year ? m.id === memory.id : m.isCover,
      }));
    ...
    setSelectedDayMemories(prev => prev
      ? prev.map(m => ({ ...m, isCover: m.id === memory.id }))
      : null);
    setMenuMemory(null);
    Alert.alert('Cover set! ⭐', `This photo is now the ${memory.year} card cover.`);
  };
  ...
  const openVaultEditSheet = async () => {
    ...
    const freshContext = contextRaw ? JSON.parse(contextRaw) : emptyContext;
    setSelectedVaultDay(prev => prev ? { ...prev, context: freshContext } : null);
    ...
    <View style={styles.outerContainer}>
        ...
        <View style={styles.container}>
          {vaultLoading ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtitle}>Open a year card, fill in the context and tap Save Day to Vault.</Text>
            </View>
          ) : (
            <>
                        ...
                        {Array.from({ length: daysInMonth }).map((_, i) => {
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              style={[
                                styles.calCell,
                                hasPhoto ? styles.calCellFilled : styles.calCellEmpty,
      ...
      <Modal visible={selectedDayMemories !== null} animationType="slide">
        <View style={styles.dayModal}>
            ...
            <View style={styles.dayContent}>
              <View style={styles.dayTitleRow}>
                ...
                <Text style={styles.dayDateText}>
                  {selectedDayDate ? new Date(selectedDayDate + 'T12:00:00').toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' }) : ''}
                </Text>
              </View>

              <TouchableOpacity style={[styles.saveDayButton, daySaved && styles.saveDayButtonSaved]} onPress={saveWholeDay}>
                <Text style={styles.saveDayButtonText}>{daySaved ? '✅ Day Saved to Vault' : '🔒 Save Day to Vault'}</Text>
              </TouchableOpacity>
              ...
              {contextFields.map(field => (
                <TouchableOpacity key={field.key} style={styles.contextCard} onPress={() => {
              ...
              <Text style={styles.sectionSubtitle}>Tap a photo to view fullscreen. Add a caption to save to Your Vault.</Text>

              {selectedDayMemories?.map((memory) => (
                <View key={memory.id} style={styles.dayPhotoCard}>
                  <TouchableOpacity style={styles.photoMenuButton} onPress={() => setMenuMemory(memory)}>
                    <Text style={styles.photoMenuDots}>•••</Text>
                  </TouchableOpacity>
                  {memory.isCover && (
                    <View style={styles.coverBadge}><Text style={styles.coverBadgeText}>⭐ Cover</Text></View>
                  )}
                  <TouchableOpacity onPress={() => { setFullScreenUri(memory.uri); setFullScreenCaption(memory.caption); }}>
                    {memory.uri
                      ? <Image source={{ uri: memory.uri }} style={styles.dayPhotoImage} />
                      : <View style={[styles.dayPhotoImage, styles.videoPlaceholder]}><Text style={styles.videoPlaceholderText}>▶ Video</Text></View>}
                  </TouchableOpacity>
                  <View style={styles.dayPhotoInfo}>
                    {memory.isScreenshot && <Text style={styles.dayPhotoType}>📸 Screenshot</Text>}
                    {memory.caption ? <Text style={styles.dayPhotoCaption}>"{memory.caption}"</Text> : null}
                    ...
                    <TouchableOpacity style={[styles.addCaptionButton, memory.caption ? styles.addCaptionButtonSaved : null]} onPress={() => openCaptionModal(memory)}>
                      <Text style={styles.addCaptionText}>{memory.caption ? '✅ Saved to Vault · Edit caption' : '🔒 Save photo to Vault'}</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              ))}
          ...
          <TouchableOpacity style={styles.dayModalClose} onPress={() => setSelectedDayMemories(null)}>
          ...
          <Modal visible={fullScreenUri !== null} transparent animationType="fade">
            <TouchableOpacity style={styles.fullScreenOverlay} activeOpacity={1} onPress={() => { setFullScreenUri(null); setFullScreenCaption(''); }}>
              {fullScreenUri && <Image source={{ uri: fullScreenUri }} style={styles.fullScreenImage} resizeMode="contain" />}
              {fullScreenCaption ? <Text style={styles.fullScreenCaption}>"{fullScreenCaption}"</Text> : null}
      ...
      <Modal visible={selectedVaultDay !== null} animationType="slide">
        <View style={styles.vaultDayModal}>
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.vaultDayContent}>
                  ...
                  {selectedVaultDay.presentEntry.mood && (
                    <View style={styles.vaultDayChip}>
                      <Text style={styles.vaultDayChipText}>
                        {selectedVaultDay.presentEntry.mood}{selectedVaultDay.presentEntry.dayDescription ? ` — ${selectedVaultDay.presentEntry.dayDescription}` : ''}
                      </Text>
                    </View>
                  )}
                  {selectedVaultDay.presentEntry.weatherEmoji && (
                    <View style={styles.vaultDayChip}>
                      <Text style={styles.vaultDayChipText}>{selectedVaultDay.presentEntry.weatherEmoji} {selectedVaultDay.presentEntry.weatherTemp}°C</Text>
                    </View>
                  )}
                </View>
              )}
                ...
                <View style={styles.vaultDaySection}>
                  ...
                  {selectedVaultDay.presentEntry.songName ? (
                    <View style={styles.vaultDayContextRow}>
                      ...
                      <Text style={styles.vaultDayContextValue}>{selectedVaultDay.presentEntry.songName}{selectedVaultDay.presentEntry.songRating > 0 ? ` — ${selectedVaultDay.presentEntry.songRating}/10` : ''}</Text>
                    </View>
                  ) : null}
              ...
              {selectedVaultDay?.photos && selectedVaultDay.photos.length > 0 && (
                <View style={styles.vaultDaySection}>
                  <Text style={styles.vaultDaySectionTitle}>Photos ({selectedVaultDay.photos.length})</Text>
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.vaultDayPhotoStrip}>
                    {selectedVaultDay.photos.map((photo) => (
                      <View key={photo.id} style={styles.vaultDayPhotoItem}>
                        ...
                        {photo.caption ? <Text style={styles.vaultDayPhotoCaption} numberOfLines={2}>"{photo.caption}"</Text> : null}
                        {photo.people.length > 0 && <Text style={styles.vaultDayPhotoPeople}>👤 {photo.people.join(', ')}</Text>}
                      </View>
                    ))}
                  </ScrollView>
                </View>
          ...
          <View style={styles.vaultDayActions}>
            ...
            <TouchableOpacity style={styles.dayModalClose} onPress={() => setSelectedVaultDay(null)}>
          ...
          <View style={styles.vaultEditBox}>
            ...
            <ScrollView>
              {contextFields.map(field => {
                ...
                return (
                  <TouchableOpacity key={field.key} style={styles.vaultEditField} onPress={() => {
      ...
      <Modal visible={showPersonProfile !== null} animationType="slide">
        <View style={styles.personModal}>
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={styles.personProfileHeader}>
              <TouchableOpacity style={styles.personProfileAvatarWrap} onPress={() => showPersonProfile && savePersonProfilePhoto(showPersonProfile)}>
                {showPersonProfile && personProfilePhotos[showPersonProfile] ? (
                  <Image source={{ uri: personProfilePhotos[showPersonProfile] }} style={styles.personProfilePhoto} />
                ) : (
                  <View style={styles.personProfileAvatarInner}>
                    <Text style={styles.personProfileInitial}>{showPersonProfile?.charAt(0).toUpperCase()}</Text>
                  </View>
                )}
                <View style={styles.personProfileEditBadge}>
                  <Text style={styles.personProfileEditText}>📷</Text>
                </View>
              </TouchableOpacity>
              <Text style={styles.personProfileName}>{showPersonProfile}</Text>
              <Text style={styles.personProfileDays}>
                {showPersonProfile ? (personDaysCount[showPersonProfile] || 0) : 0} days documented together
              </Text>
            </View>

            {personPhotos.length > 0 && (
              <View style={styles.personGallerySection}>
                <Text style={styles.personGalleryTitle}>Photos together</Text>
                <View style={styles.personGalleryGrid}>
                  {personPhotos.map((photo, i) => (
                    <View key={i} style={styles.personGalleryItem}>
                      ...
                      {photo.caption ? <Text style={styles.personGalleryCaption} numberOfLines={2}>"{photo.caption}"</Text> : null}
                    </View>
                  ))}
                </View>
              </View>
          ...
          <TouchableOpacity style={styles.dayModalClose} onPress={() => setShowPersonProfile(null)}>
...
const styles = StyleSheet.create({
  ...
  calCell: { width: CARD_WIDTH, aspectRatio: 2 / 3, borderRadius: 6, overflow: 'hidden' },
  calCellFilled: { borderWidth: 1.5, borderColor: '#ffffff' },
  calCellEmpty: { borderWidth: 1, borderColor: 'rgba(255,255,255,0.25)', alignItems: 'center', justifyContent: 'center' },
  calCellToday: { borderWidth: 2, borderColor: '#ffffff' },
  calDayNumFilled: { position: 'absolute', bottom: 4, left: 6, color: '#ffffff', fontSize: 13, fontWeight: '800', textShadowColor: 'rgba(0,0,0,0.8)', textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3 },
  calDayNumEmpty: { color: 'rgba(255,255,255,0.5)', fontSize: 13, fontWeight: '700' },
